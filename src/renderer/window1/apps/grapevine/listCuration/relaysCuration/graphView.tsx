...
const GraphInit = ({aNodes, oMyNostrProfileData, oNostrProfilesData, aEndorseAsRelaysPicker, aEndorseAsRelaysPickerHunter}) => {
  const nodes_arr = [];
  const edges_arr = [];

  let defaultImageUrl = 'https://nostr.build/i/2282.png';

  // ADD NODES
  let oKind0ProfilesData = {};
  for (let n=0;n<aNodes.length;n++) {
    const pk = aNodes[n]
    const oNode = { id: pk, pubkey: pk, shape: 'circularImage', image: defaultImageUrl, brokenImage: defaultImageUrl }
  ...
  for (let e=0;e<aEndorseAsRelaysPicker.length;e++) {
    ...
    const oEdge = {from:aRating[0], to: aRating[1], color: 'green', title: 'Endorse Relays'}
    edges_arr.push(oEdge)
  }

  for (let e=0;e<aEndorseAsRelaysPickerHunter.length;e++) {
    const aRating = aEndorseAsRelaysPickerHunter[e]
    const oEdge = {from:aRating[0], to: aRating[1], color: 'black', title: 'Endorse as Relays Curator Hunter'}
    edges_arr.push(oEdge)
  }
  ...
  return (
    <>
      <div style={{ height: '500px', width: '100%', border: '1px solid purple' }} ref={domNode}>
        Hello
      </div>
    </>
  );
...
const GraphView = ({oMyNostrProfileData, oNostrProfilesData}) => {
  ...
  const oPickerHunter = listCuration.relays.ratings.notes.endorseAsRelaysPickerHunter;
