...
const decodeAndSend = async (event, myPubkey, myPrivKey, pubkey) => {
  try {
...
const MessageZ = ({ event }) => {
  ...
  try {
    ...
    decodeAndSend(event, myPubkey, myPrivKey, pubkey);
  } catch (err) {}
      ...
      <div className="eventContainer">
        <div className={directMessageContainerClassName}>
          <div className="directMessageContentContainer">
            <pre className={devModeClassName}>{JSON.stringify(event,null,4)}</pre>
            <div id={event.id} />
