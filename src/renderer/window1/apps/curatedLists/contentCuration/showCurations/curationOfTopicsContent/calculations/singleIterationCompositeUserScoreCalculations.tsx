...
import {
  convertInputToCertainty,
  convertRatingToMod3Coeff,
} from 'renderer/window1/lib/grapevine';
import { nodes, edges, aAllUserNodes } from '../curationOfTopicsContent';

export const foo = () => {};

export const singleIterationCompositeUserScoreCalculations = (
  myPubKey,
    ...
    if (oNode) {
      ...
      for (let e = 0; e < afferentEdgeIDs.length; e++) {
        ...
        if (aAllUserNodes.includes(pk_from) && aAllUserNodes.includes(pk_to)) {
          ...
          const mod1Coeff =
            raterCurrentAverage * strat1Coeff_directRating +
            1 * (1 - strat1Coeff_directRating);
      ...
      const weight_default =
        attenuationFactor_default *
        mod3Coeff_default *
        strat2Coeff_default *
        strat1Coeff_default *
        ratingConfidence_default *
        raterCurrentInfluence_default;
      ...
      oNode.scores.thisListCuration_allContexts.defaultScore.product =
        parseFloat(product_default.toPrecision(4));
      oNode.scores.thisListCuration_allContexts.defaultScore.weight =
        parseFloat(weight_default.toPrecision(4));
      oNode.scores.thisListCuration_allContexts.defaultScore.weightAdjusted =
        parseFloat(weightAdjusted_default.toPrecision(4));
      // oNode.scores.thisListCuration_allContexts.defaultScore.weightAdjusted = weight_default - sumOfWeights_directRatings;

      oNode.scores.thisListCuration_allContexts.sumOfProducts =
        sumOfProducts.toPrecision(4);
      // oNode.scores.thisListCuration_allContexts.sumOfWeights = oNode.name;
      oNode.scores.thisListCuration_allContexts.sumOfWeights =
        sumOfWeights.toPrecision(4);
