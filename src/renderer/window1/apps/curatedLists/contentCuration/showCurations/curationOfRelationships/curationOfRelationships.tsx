...
const makeVisGraph = async (
  ...
  const oNostrRelationshipSlugs = oNostrNodes.byWordType.relationship
  const aNostrRelationshipSlugs = Object.keys(oNostrRelationshipSlugs);
  for (let x = 0; x < aNostrRelationshipSlugs.length; x++) {
    const nostrRelationshipSlug = aNostrRelationshipSlugs[x];
    const nostrRelationshipEventID = oNostrNodes.bySlug[nostrRelationshipSlug].versionIndependent;
    const oWord = oNostrNodes.byEventID[nostrRelationshipEventID].word;
    console.log("oWord: "+JSON.stringify(oWord,null,4));
    if (oWord.hasOwnProperty(propertyPath)) {
      const nF_slug = oWord[propertyPath]?.nodeFrom.slug;
      const rT_name = oWord[propertyPath]?.relationshipType.name;
      const nT_slug = oWord[propertyPath]?.nodeTo.slug;
      const item_name = nF_slug + " " + rT_name + " " + nT_slug;
      if (!aNodesIDs.includes(nostrRelationshipEventID)) {
        const oNode = {
          id: nostrRelationshipEventID,
          group: 'instance',
          title: item_name,
          label: item_name,
          name: item_name,
        ...
        nodes_arr.push(oNode);
        aNodesIDs.push(nostrRelationshipEventID);
      }
    }
  }
  ...
  for (let x=0;x<aInstanceRatingEventIDs.length;x++) {
    ...
    if (!aEdgesIDs.includes(event_id)) {
      ...
      if (oWord.hasOwnProperty("ratingData")) {
        if (oWord.ratingData.ratingTemplateData.ratingTemplateSlug == "nostrChannelTopicsRelationshipInstanceEndorsement") {
          console.log("qwerty B");
          const pk_rater = oWord.ratingData.raterData.nostrProfileData.pubkey;
          const event_id_ratee = oWord.ratingData.rateeData.nostrChannelTopicRelationshipData.eventID;
          const regularSliderRating = oWord.ratingData.ratingFieldsetData.nostrChannelTopicsRelationshipInstanceEndorsementFieldsetData.regularSliderRating;
          let title = '';
...
export default class CurationOfRelationships extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
