...
export const extractNodesAndEdgesRedo = (
  oMyNostrProfileData,
  oNostrProfilesData,
  propertyPath,
  oCuratedListData,
  ...
  const aItemEventIDs = Object.keys(oCuratedListData.items);

  for (let x=0; x<aItemEventIDs.length; x++) {
    const item_event_id = aItemEventIDs[x];
    const oWord = oCuratedListData.items[item_event_id].oWord;
    if (oWord.hasOwnProperty(propertyPath)) {
      ...
      if (!aNodesIDs.includes(item_event_id)) {
        const oNode = {
          id: item_event_id,
          group: 'instance',
          title: item_name,
  ...
  const aCuratorPubkeys = Object.keys(oCuratedListData.curators);

  for (let x=0; x<aCuratorPubkeys.length; x++) {
    const pubkey = aCuratorPubkeys[x];

    let curator_profilePicUrl = noProfilePicUrl;
    let curator_display_name = "";
    let curator_name = "";

    if (oNostrProfilesData[pubkey]) {
      const oEventProfile = JSON.parse(oNostrProfilesData[pubkey].event);
      const oProfileContent = JSON.parse(oEventProfile.content);
      if (oProfileContent.picture) {
        curator_profilePicUrl = oProfileContent.picture;
        curator_display_name = oProfileContent.display_name;
        curator_name = oProfileContent.name;
      }
    }

    if (!aNodesIDs.includes(pubkey)) {
      // add rater as another rater node
      let oNode = {
        id: pubkey,
        group: "user",
        shape: 'circularImage',
        image: curator_profilePicUrl,
        brokenImage: noProfilePicUrl,
        title: curator_display_name,
        label: curator_name,
        name: curator_name,
      ...
      oResults.aNodes.push(oNode);
      aNodesIDs.push(pubkey);
    }
  }

  // ADD RATINGS OF CURATORS

  for (let x=0; x<aCuratorPubkeys.length; x++) {
    const ratee_pubkey = aCuratorPubkeys[x];

    const aThumbsUpPubkeys = oCuratedListData.curators[ratee_pubkey].thumbsUp;
    const aThumbsDownPubkeys = oCuratedListData.curators[ratee_pubkey].thumbsDown;
    for (let z=0;z<aThumbsUpPubkeys.length;z++) {
      const rater_pubkey = aThumbsUpPubkeys[z];
      const endorsement_edge_id = "curatorEndorsementThumbsUp_" + z;
      const oEdge = {
        id: endorsement_edge_id,
      ...
      if (!aEdgesIDs.includes(endorsement_edge_id)) {
        oResults.aEdges.push(oEdge);
        aEdgesIDs.push(endorsement_edge_id);
      }
    }
    for (let z=0;z<aThumbsDownPubkeys.length;z++) {
      const rater_pubkey = aThumbsDownPubkeys[z];
      const endorsement_edge_id = "curatorEndorsementThumbsDown_" + z;
      const oEdge = {
        id: endorsement_edge_id,
        // group: 'ratingOfInstance', // no group styling for edges (I think ???)
        from: rater_pubkey,
  ...
  for (let x=0; x<aItemEventIDs.length; x++) {
    const item_event_id = aItemEventIDs[x];
    const aThumbsUpPubkeys = oCuratedListData.items[item_event_id].ratings.thumbsUp;
    const aThumbsDownPubkeys = oCuratedListData.items[item_event_id].ratings.thumbsDown;
    for (let z=0; z<aThumbsUpPubkeys.length; z++) {
      const rater_pubkey = aThumbsUpPubkeys[z];
      const endorsement_edge_id = "itemEndorsementThumbsUp_" + z;
      const oEdge = {
        id: endorsement_edge_id,
        // group: 'ratingOfInstance', // no group styling for edges (I think ???)
        from: rater_pubkey,
  ...
  for (let r=0;r<aRatingsOfInstancesData.length;r++) {
    ...
    if (ratingTemplateSlug=="nostrCuratedListInstanceGenericRating") {
      ...
      // ratee data
      const item_event_id = oRatingWord.ratingData.rateeData.nostrCuratedListInstanceData.eventID;
